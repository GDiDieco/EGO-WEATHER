...
def fetch_24h(conn, table: str, hours: int):
    sql = f"""
        SELECT
            ...
            MAX(COALESCE(rainRate, 0)) AS rain_rate_max
        FROM {table}
...
def fetch_7d(conn, table: str, days: int):
    sql = f"""
        SELECT
            FLOOR(dateTime / 86400) * 86400 AS bucket_ts,
            SUM(COALESCE(rain, 0)) AS rain_sum,
            MAX(COALESCE(rainRate, 0)) AS rain_rate_max
        FROM {table}
        WHERE dateTime >= UNIX_TIMESTAMP() - (%s * 86400)
        GROUP BY FLOOR(dateTime / 86400)
        ORDER BY bucket_ts ASC
    """
    with conn.cursor() as cur:
...
def build_points(rows, with_day=False):
    points = []
    cumulative = 0.0
    for r in rows:
        rain_mm = inch_to_mm(r["rain_sum"]) or 0.0
        cumulative += rain_mm
        item = {
            "ts": int(r["bucket_ts"]),
            "time": ts_to_iso_local(r["bucket_ts"]),
            "rain_mm": round(rain_mm, 2),
            "rain_rate_max_mm_h": inch_to_mm(r["rain_rate_max"]),
            "rain_cumulative_mm": round(cumulative, 2),
        }
        if with_day:
            item["day"] = r["day_key"]
        points.append(item)
...
def build_points_with_label(rows, label_key=None):
    points = []
    cumulative = 0.0
    for r in rows:
        rain_mm = inch_to_mm(r["rain_sum"]) or 0.0
...
def main() -> int:
    ...
    try:
        ...
        output_path = public_data_dir / "history-rain.json"

        table = cfg["history"]["db"].get("table", "archive")
        ranges_cfg = cfg["history"].get("ranges", {})

        hours_24h = int(ranges_cfg.get("rain_24h_hours", 24))
        days_7d = int(ranges_cfg.get("rain_7d_days", 7))
        days_30d = int(ranges_cfg.get("rain_30d_days", 30))

        days_1y = int(ranges_cfg.get("rain_1y_days", 365))
        years_5y = int(ranges_cfg.get("rain_5y_years", 5))
        ...
        payload["ranges"] = {
            "24h": build_points(rows_24h, with_day=False),
            "7d": build_points(rows_7d, with_day=False),
            "30d": build_points(rows_30d, with_day=True),
            "1y": build_points_with_label(rows_1y, label_key="day_key"),
            "5y": build_points_with_label(rows_5y, label_key="month_key"),
        }
        ...
        if output_path is None:
            try:
                cfg = load_config()
                output_path = Path(cfg["paths"]["public_data_dir"]) / "history-rain.json"
            except Exception:
                output_path = Path("history-rain.json")
        previous = read_json_file(output_path)
        fallback = merge_stale(previous, f"Aggiornamento storico pioggia fallito: {e}")
